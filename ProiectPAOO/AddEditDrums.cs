...
{
    public partial class AddEditDrums : Form
    {
        public AddEditDrums()
        {
            InitializeComponent();
        ...
        internal void EditInstrument(Instruments instr, Drums drum)
        {
            this.Text = "Edit Instrument";
            label1.Text = "Edit Drums";
            ...
            textBox6.Text = drum.DrumType;
            numericUpDown1.Value = drum.NumberOfDrums;
            checkBox1.CheckState = drum.IsElectric ? CheckState.Checked : CheckState.Unchecked;
        }
        ...
        private void button2_Click(object sender, EventArgs e) //Save
        {
            ...
            {
                DrumType = textBox6.Text,
                NumberOfDrums = Convert.ToInt32(numericUpDown1.Value),
                IsElectric = Convert.ToBoolean(checkBox1.CheckState)
            };
            var repo = new InstrumentRepository();
            if (instrumentId == 0)
            {
                repo.AddDrums(instrument, drum);
            }           
            else
            {
                repo.UpdateDrums(instrument, drum);
            }
